...
var ContentFlowItem  = function (CFobj, element, index) {
    ...
    if ($CF(element).nodeName == "IMG"||$CF(element).nodeName == "PNG") {
    ...
    if (this.content.nodeName == "IMG"||this.content.nodeName == "PNG") {
        CFobj._imagesToLoad++;
...
*/
	...
	onDragWithGesture: function (steps){
		var dim = this.Flow.dimensions.width;
		var itemDist = steps;//(distance / dim )* (2*this.conf.visibleItems +1); // items
